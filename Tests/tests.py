import os
import os.path as pt
import subprocess
import random as rnd
import re
import shutil

VERBOSE = False

CLEANUPLIST = ("ShaderData.cpp", "ShaderData.hpp", "ShaderPipes.cpp",
               "ShaderPipes.hpp")


def cleanup():
    if os.path.exists("Resources"): shutil.rmtree("Resources")
    for x in CLEANUPLIST:
        if os.path.exists(x): os.remove(x)


def runcommand(command, console, toolpath):
    '''
    Runs a command in the given mode
    1 for console only
    2 for arguments only
    3 for both
    '''
    result = [0, 0]
    if console & 1 == 1:
        pr = subprocess.Popen(toolpath,
                              stdout=subprocess.PIPE,
                              stdin=subprocess.PIPE,
                              stderr=subprocess.PIPE)
        outs, errs = pr.communicate(bytes(command + "\nexit\n", "ascii"))
        outstr = str(outs)
        if "Error " in outstr:
            result[0] = int(
                re.split(r"(\\r|\\n)",
                         outstr.split("Error ")[1])[0].strip())
        pr.wait()
    if console & 2 == 2:
        if VERBOSE:
            rtc = subprocess.call(toolpath + " " + command)
        else:
            rtc = subprocess.call(toolpath + " " + command,
                                  stdout=subprocess.PIPE,
                                  stdin=subprocess.PIPE,
                                  stderr=subprocess.PIPE)
        if rtc > 2**31 - 1:
            # Convert to signed int
            rtc -= 2**32
        result[1] = rtc
    return tuple(result)


def rngd(count=1):
    result = str()
    for x in range(count):
        result += str(rnd.random())
        if x + 1 != count:
            result += " "
    return result

def rngn(count=1):
    result = str()
    for x in range(count):
        result += str(rnd.randint(0, 1000000))
        if x + 1 != count:
            result += " "
    return result

def rng(regex=r"[\W]", size=0):
    if size == 0:
        size = rnd.randint(3, 20)
    resultarray = str()
    while len(resultarray) < size:
        resultarray += re.sub(regex,
                              "",
                              str(chr(rnd.randint(33, 126))),
                              flags=re.M)
    if len(resultarray) > size:
        resultarray = resultarray[:size]
    return resultarray


HEXREGEX = r"[^a-fA-F0-9]"
NONEWORD = r"[\w\"\s]"

TESTS = [
    # General
    (rng(), 3, (1, 1)),
    #("setcwd", 3, (1, 1)),
    #("settool", 3, (0, 0)),
    #("setcwd " + pt.abspath(os.getcwd()), 3, (0, 0)),
    # Pipeline tests
    ("pipe", 3, (1, 1)),
    ("pipe create", 3, (1, 1)),
    ("pipe delete", 3, (1, 1)),
    ("pipe show", 3, (1, 1)),
    ("pipe update", 3, (1, 1)),
    #("pipe list", 3, (0, 0)),
    #("pipe make", 3, (0, 0)),
    ("pipe " + rng(), 3, (1, 1)),
    # Shader tests
    ("shader", 3, (1, 1)),
    #("shader make", 3, (0, 0)),
    #("shader list", 3, (0, 0)),
    ("shader compile", 3, (1, 1)),
    ("shader " + rng(), 3, (1, 1)),
    # Texture tests
    ("texture add", 3, (1, 1)),
    ("texture add " + rng(), 3, (3, 3)),
    ("texture add test.png", 1, (0, 0)),
    ("texture remove test", 1, (0, 0)),
    ("texture add test.png", 2, (0, 0)),
    ("texture remove test", 2, (0, 0)),
    ("texture add test.png", 2, (0, 0)),
    ("texture add test.png", 3, (1, 1)),
    ("texture remove", 3, (1, 1)),
    ("texture remove " + rng(), 3, (1, 1)),
    ("texture list", 3, (0, 0)),
    ("texture " + rng(), 3, (1, 1)),
    # Material Tests
    ("material", 3, (1, 1)),
    ("material " + rng(), 3, (1, 1)),
    ("material add", 3, (1, 1)),
    ("material add " + rng(), 3, (1, 1)),
    ("material add " + rng(NONEWORD) + " test", 3, (1, 1)),
    ("material add test " + rng(), 3, (1, 1)),
    ("material add test test", 1, (0, 0)),
    ("material rm test", 1, (0, 0)),
    ("material add test test", 2, (0, 0)),
    ("material rm test", 2, (0, 0)),
    ("material rm " + rng(), 3, (1, 1)),
    ("material remove " + rng(), 3, (1, 1)),
    ("material add test test", 1, (0, 0)),
    ("material remove test", 1, (0, 0)),
    ("material add test test " + rng(), 3, (1, 1)),
    ("material add testmaterial test", 1, (0, 0)),
    ("material add testmaterial test", 3, (1, 1)),
    ("material add test2 test " + rng(HEXREGEX, 6), 1, (0, 0)),
    ("material add test3 test " + rng(HEXREGEX, 6), 2, (0, 0)),
    ("material rm", 3, (1, 1)),
    ("material remove", 3, (1, 1)),
    ("material settexture", 3, (1, 1)),
    ("material settexture test", 3, (1, 1)),
    ("material settexture test2 test", 3, (0, 0)),
    ("material settexture " + rng() + " test", 3, (1, 1)),
    ("material settexture test2 " + rng(), 3, (1, 1)),
    ("material settex", 3, (1, 1)),
    ("material settex test", 3, (1, 1)),
    ("material settex test2 test", 3, (0, 0)),
    ("material settex " + rng() + " test", 3, (1, 1)),
    ("material settex test2 " + rng(), 3, (1, 1)),
    ("material setcolor", 3, (1, 1)),
    ("material setcolor " + rng(), 3, (1, 1)),
    ("material setcolor test3 " + rng(), 3, (1, 1)),
    ("material setcolor test3 " + rng(HEXREGEX, 6), 3, (0, 0)),
    ("material list", 3, (0, 0)),
    # Actor tests
    ("actor", 3, (1, 1)),
    ("actor add", 3, (1, 1)),
    ("actor add " + rng(), 3, (1, 1)),
    ("actor add testactor testmaterial", 1, (0, 0)),
    ("actor add " + rng(NONEWORD) + " testmaterial", 3, (1, 1)),
    ("actor add testactor2 testmaterial", 2, (0, 0)),
    ("actor add testactor2 testmaterial", 3, (1, 1)),
    ("actor add testactor " + rng(), 3, (1, 1)),
    ("actor rm testactor", 1, (0, 0)),
    ("actor rm testactor2", 2, (0, 0)),
    ("actor rm testactor", 3, (1, 1)),
    ("actor add testactor testmaterial", 1, (0, 0)),
    ("actor add testactor2 testmaterial", 2, (0, 0)),
    ("actor remove testactor", 1, (0, 0)),
    ("actor remove testactor2", 2, (0, 0)),
    ("actor remove testactor", 3, (1, 1)),
    ("actor add testactor testmaterial", 1, (0, 0)),
    ("actor vertex testactor ", 3, (1, 1)),
    ("actor rm", 3, (1, 1)),
    ("actor remove", 3, (1, 1)),
    ("actor list", 3, (0, 0)),
    ("actor vertex", 3, (1, 1)),
    ("actor index", 3, (1, 1)),
    ("actor transform", 3, (1, 1)),
    ("actor layer", 3, (1, 1)),
    ("actor instance", 3, (1, 1)),
    ("actor vertex " + rng(), 3, (1, 1)),
    ("actor index " + rng(), 3, (1, 1)),
    ("actor transform " + rng(), 3, (1, 1)),
    ("actor layer " + rng(), 3, (1, 1)),
    ("actor instance " + rng(), 3, (1, 1)),
    ("actor vertex " + rng() + " 1.0", 3, (1, 1)),
    ("actor index " + rng() + " 1", 3, (1, 1)),
    ("actor transform " + rng() + " 0", 3, (1, 1)),
    ("actor layer " + rng() + " 0", 3, (1, 1)),
    ("actor instance " + rng() + " " + rng(), 3, (1, 1)),
    ("actor " + rng(), 3, (1, 1)),
    # Map tests
    ("map", 3, (1, 1)),
    ("map add", 3, (1, 1)),
    ("map rm", 3, (1, 1)),
    ("map remove", 3, (1, 1)),
    ("map make", 3, (1, 1)),
    ("map list", 3, (0, 0)),
    ("map addactor", 3, (1, 1)),
    ("map rmactor", 3, (1, 1)),
    ("map removeactor", 3, (1, 1)),
    ("map addfont", 3, (1, 1)),
    ("map rmfont", 3, (1, 1)),
    ("map removefont", 3, (1, 1)),
    ("map make " + rng(), 3, (1, 1)),
    ("map add " + rng(), 2, (0, 0)),
    ("map add " + rng(NONEWORD), 3, (1, 1)),
    ("map add test", 1, (0, 0)),
    ("map add test", 3, (1, 1)),
    ("map rm test", 1, (0, 0)),
    ("map add test", 2, (0, 0)),
    ("map rm test", 2, (0, 0)),
    ("map add test", 2, (0, 0)),
    ("map remove test", 2, (0, 0)),
    ("map add test", 2, (0, 0)),
    ("map remove test", 2, (0, 0)),
    ("map remove " + rng(), 3, (1, 1)),
    ("map " + rng(), 3, (1, 1)),
    # Font tests
    ("font", 3, (1, 1)),
    ("font add", 3, (1, 1)),
    ("font rm", 3, (1, 1)),
    ("font remove", 3, (1, 1)),
    ("font list", 3, (0, 0)),
]

FAILED = 0
RUNS = 0


def runtest(toolpath):
    global FAILED
    global RUNS
    RUNS += 1
    cleanup()
    for test in TESTS:
        output = runcommand(test[0], test[1], toolpath)
        if output != test[2]:
            print("Failed at " + str(test) + " Output: " + str(output))
            FAILED += 1


for path, name, files in os.walk("../"):
    if "TGToolsConsole.exe" in files:
        toolpath = pt.abspath(pt.join(path,
                                      "TGToolsConsole.exe")).replace("\\", "/")
        runtest(toolpath)
cleanup()

if RUNS == 0:
    raise Exception("No executable found")

if FAILED != 0:
    raise Exception(
        str(FAILED) + " out of " + str(len(TESTS) * RUNS) + " tests failed!")
else:
    print("Everything passed!")